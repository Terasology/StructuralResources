...
public class IngotStackSystem extends BaseComponentSystem {
    ...
    public void onRightClick(ActivateEvent event, EntityRef entity, IngotComponent ingotComponent) {
        ...
        if (stackComponent != null && stackComponent.ingots < MAX_INGOTS) {
            ...
            instigator.send(new PlaySoundEvent(Assets.getSound("engine:PlaceBlock").get(), 0.5f));
            inventoryManager.moveItem(instigator, instigator, findSlot(instigator),
                    blockEntityRegistry.getBlockEntityAt(targetPos), 0, 1);
    ...
    @ReceiveEvent
    public void onStackSizeChange(InventorySlotStackSizeChangedEvent event, EntityRef stackEntity,
                                  IngotStackComponent stackComponent) {
        ...
        if (event.getNewSize() > MAX_INGOTS) {
            inventoryManager.moveItem(stackEntity, instigator, 0, instigator, findSlot(instigator),
                    event.getNewSize() - MAX_INGOTS);
        }
